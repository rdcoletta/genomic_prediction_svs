...
optional argument:
  ...
  --error-bars=[VALUE]          error bars can be either 'CI' (confidence intervals; default) or 'SE' (standard error)
...
for (qtn_number in trait_qtn_number) {
  for (heritability in trait_heritability) {
    ...
    results_plot <- ggplot(data = prediction_summary_plot,
                           aes(x = predictor, y = mean_accuracy_envs, fill = interaction(predictor, cv))) +
      ...
      scale_fill_manual(values = c("#866BA0FF", "#BA7BA5FF", "#E7948BFF", "#F6AB77FF", "#FDC66AFF",
                                   "#360961FF", "#8C2369FF", "#D84D3EFF", "#F1731DFF", "#FCA108FF"),
                        breaks = c("SNPs\nand SVs.CV1", "SNPs\nand SVs.CV2"),
                        labels = c("CV1", "CV2")) +
      guides(fill = guide_legend("Cross-Validation", override.aes = list(fill = c("gray60", "gray20")))) +
    ...
    # save plot
    plot_name <- paste0(outfolder, "/pred_accuracy.with_gxe.qtns_", qtn_number, ".h2_", heritability, ".pdf")
...
for (qtn_number in trait_qtn_number) {
  for (heritability in trait_heritability) {
    for (var_ratio in snp_sv_ratio) {
      ...
      # save plot
      plot_name <- paste0(outfolder, "/pred_accuracy.with_gxe.qtns_", qtn_number, ".h2_", heritability,
