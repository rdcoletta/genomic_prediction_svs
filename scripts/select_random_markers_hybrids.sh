...
for marker in all sv snp snp_ld snp_not_ld; do
  ...
  mkdir -p analysis/trait_sim_hybrids/datasets/iter${ITER}
  # set input variables
  IN=analysis/trait_sim_hybrids/datasets/usda_hybrids.${marker}_markers.hmp.txt
  OUT=analysis/trait_sim_hybrids/datasets/iter${ITER}/usda_hybrids.${marker}_markers.adjusted-n-markers.hmp.txt
  OPT=${options}
  SEED=${SEED}
  # sample markers
  ...
  run_pipeline.pl -Xmx10g -importGuess analysis/trait_sim_hybrids/datasets/iter${ITER}/usda_hybrids.${marker}_markers.adjusted-n-markers.hmp.txt \
                  -export analysis/trait_sim_hybrids/datasets/iter${ITER}/usda_hybrids.${marker}_markers.adjusted-n-markers.hmp.txt \
