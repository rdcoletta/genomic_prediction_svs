...
# sv.folder <- "data/hapmap_by_cross"
...
for (row in 1:NROW(df.crosses)) {
  ...
  if (length(filename.after.proj) > 0) {
    ...
    sv.filename <- list.files(path = sv.folder,
                              pattern = paste0("usda_SVs_parents.sorted.", cross, ".hmp.txt"),
                              full.names = TRUE)
    # open file with SV positions
    sv.file <- fread(sv.filename, header = TRUE, data.table = FALSE)
    # filter out missing data in both parents
    sv.file <- sv.file[which(sv.file[, 12] != "NN" & sv.file[, 13] != "NN"), ]
...
outfile.sum <- paste0(proj.folder, "/projection_svs_summary.txt")
