...
prediction_summary <- prediction_results %>%
  ...
  summarize(pops = n(),
            ...
            accuracy_lowerCI = mean_accuracy_envs - (qt(p = 0.05/2, df = (pops * 5) - 1, lower.tail = FALSE) * accuracy_se),
            accuracy_upperCI = mean_accuracy_envs + (qt(p = 0.05/2, df = (pops * 5) - 1, lower.tail = FALSE) * accuracy_se))
