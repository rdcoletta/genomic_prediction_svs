...
file_qtn_info <- list.files(sim_folder, pattern = "Additive_QTNs", full.names = TRUE, recursive = TRUE)
...
for (env in 1:length(files_var_qtn)) {
  ...
  qtn_effects_env <- qtn_info[, paste0("add_eff_t", env)]
...
# sim_folder <- "analysis/trait_sim/multi_env/with_gxe/additive_model/equal_effects/100-QTNs_from_SNP/0.7-heritability/pop1"
