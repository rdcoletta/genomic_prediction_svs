...
Test accuracy imputation only with SNPs. Use dataset with missing data and hets (i.e. before rqtl filtering) and also use dataset with SNPs used in rqtl (get list of markers from `recomb_freq_{cross}_rils.txt` files).



<mark>TO DO</mark>:
* Test accuracy of imputation: use only SNPs here; randomly remove some markers from RILs and perform imputation; compare results with what it was supposed to be.
* Filter out SNPs that fall inside boundaries of SV.
* For PHG35, see how many SV calls there are, and if numbers are very different than other parents.
